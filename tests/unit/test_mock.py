...
PDF_PATH = root_dir / "tests" / "test_files" / "test.pdf"

app = RapidVLMOCR(engine=EngineType.MOCK)


def test_mock_single_image_inference_returns_expected_response() -> None:
    input_path = TEST_FILES_DIR / "general.jpg"

    response = app.run(
        TaskType.TEXT_EXTRACTION, input_path=input_path, metadata={"page": 1}
    )
    ...
    assert response.metadata["input_path"] == str(input_path.resolve())
    assert response.metadata["page"] == 1


def test_mock_batch_inference_returns_expected_responses() -> None:
    input_paths = [TEST_FILES_DIR / "general.jpg", TEST_FILES_DIR / "general_1.jpeg"]
    ...
    for idx, response in enumerate(responses, start=1):
        ...
        assert response.raw_output == "mock text extraction"
        assert response.parsed_output == "mock text extraction"
        assert response.metadata["input_path"] == str(input_paths[idx - 1].resolve())
...
def test_mock_document_parse_returns_expected_markdown() -> None:
    input_path = TEST_FILES_DIR / "document.png"

    response = app.run(TaskType.DOCUMENT_PARSING, input_path=input_path)
...
def test_mock_kie_returns_expected_json() -> None:
    input_path = TEST_FILES_DIR / "invoice.jpg"

    response = app.run(TaskType.KIE, input_path=input_path)
...
def test_run_rejects_pdf_input() -> None:
    with pytest.raises(
        ValueError, match="PDF input is only supported via __call__ or run_batch"
    ):
        app.run(TaskType.DOCUMENT_PARSING, input_path=PDF_PATH)


def test_mock_pdf_batch_inference_returns_page_responses() -> None:
    responses = app.run_batch(
        TaskType.DOCUMENT_PARSING,
        input_paths=[PDF_PATH],
        metadata_list=[{"doc_id": 7}],
        batch_size=4,
    )
    ...
    for idx, response in enumerate(responses, start=1):
        ...
        assert response.task_type == TaskType.DOCUMENT_PARSING
        assert response.raw_output == "# Mock Document\n\nThis is a mock markdown output."
        assert response.metadata["input_path"] == str(PDF_PATH.resolve())
        assert response.metadata["doc_id"] == 7
        assert response.metadata["page"] == idx
